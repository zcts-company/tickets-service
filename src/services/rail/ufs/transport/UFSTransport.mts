...
export class UFSTransport {
            ...
            if(files.length > 0){
                files.forEach(async (fileName) => {
                    try{
                            ...
                            if(exists){
                                logger.info(`[UFS TRANSPORT] File ${fileName} sended to directory: ${this.directory1C}`);
                                await this.sendToArchive(currentArchive,fileName)
                            }
                    }catch {
                        logger.error(`[UFS TRANSPORT] Directory ${this.directory1C} not exists or not available`)                        
                    }
            })
           }
                 ...
                 files.forEach(async (fileName) => {
                     try{
                        ...
                        const exists:boolean = await this.sambaClient.fileExists(config.samba.directory + fileName)
                             ...
                             if(exists){
                                 logger.info(`[UFS TRANSPORT] File ${this.currentDirectory + fileName} sended to directory (SAMBA SERVER:${config.samba.server}): ${config.samba.directory}`);
                                 await this.sendToArchive(currentArchive,fileName)
                             }
                     }catch (error:any) {
                         logger.error(`[UFS TRANSPORT] Directory (SAMBA SERVER:${config.samba.server}) ${config.samba.directory} not exists or not available`)
                         logger.error(`[UFS TRANSPORT] ERROR: ${error.message}`)
                     }
             })
            }
    ...
    private async sendToArchive(currentArchive:string,fileName:string){
        ...
        if(exists){
            logger.info(`[UFS TRANSPORT] File ${fileName} sended to archive directory: ${currentArchive}`);
    ...
    private async removeFileFromCurrent(fileName:string){
       ...
       const exist:boolean = await fs.pathExists(`${this.currentDirectory}${fileName}`)
            ...
            if(!exist){
                logger.info(`[UFS TRANSPORT] File ${fileName} removed from current directory: ${this.currentDirectory}`);
            }
    }
